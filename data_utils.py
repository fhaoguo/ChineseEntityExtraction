...
class BatchManager(object):
    ...
    def __init__(self, data, batch_size, model_type="bi_lstm"):
        if model_type == "bi_lstm":
            self.batch_data = self.sort_and_pad(data, batch_size)
        elif model_type == "bert":
            self.batch_data = self.sort_and_pad_for_bert(data, batch_size)
        else:
            raise Exception("Unknown model type or missing!")
    ...
    def sort_and_pad_for_bert(self, data, batch_size):
        num_batch = math.ceil(len(data) / batch_size)
        sorted_data = sorted(data, key=lambda x: len(x[0]))
        batch_data = list()
        for i in range(num_batch):
            batch_data.append(self.arrange_batch(sorted_data[i * batch_size:(i + 1) * batch_size]))
        return batch_data
    ...
    def pad_data(data):
        ...
        segs = []
        targets = []
        max_length = len(data[len(data)-1][0])
        for line in data:
